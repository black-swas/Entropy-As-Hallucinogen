...
1. Just use log-probability (the normal baseline)
2. Just use entropy (pick the lowest-entropy answer)
3. A simple rule that switches between the two
4. A tuned blend of both
5. An "oracle" — the best possible outcome if I always picked whichever signal happened to be right for that question. This isn't a real method, it's a ceiling to measure the others against.

## What happened

| Method | Accuracy |
|---|---|
| Log-probability only (baseline) | 25.6% |
| Entropy only | 23.4% |
| Rule-based switch | 24.8% |
| Best tuned blend | 26.0% |
| Oracle (best possible) | 40.8% |
